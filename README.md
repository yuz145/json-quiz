...
- シャッフル出題、回答形式切り替え（「答えを見る」/「文字入力」）、「文字入力」モード時のみ現れる「完全一致で自動正解」のトグルボタンを大型化＆ON/OFFで明確に色付け。設定はブラウザに保存され次回アクセス時も引き継がれる
- 入力して採点モードでは、完全一致の自動判定を色付きバッジ（緑=一致／赤=不一致）で分かりやすく表示。「完全一致で自動正解」がONの場合、完全一致した時点で「正解した」ボタンを押さずに自動的に正解として確定する（不一致の場合は従来通り手動で正解/不正解を判定）
...
| 状態 | キー | 動作 |
| --- | --- | --- |
| 「文字入力」モードで回答を入力中 | `Enter` | 回答を送信（`Space`はそのまま文字として入力されます） |
| 送信後、完全一致の自動判定バッジが出ている | `Enter` | 自動判定の結果をそのまま確定して次の問題へ（一致なら正解、不一致なら不正解） |
| 同上（自動判定を手動で上書きしたい場合） | `F` | 正解として確定して次の問題へ |
| 同上 | `J` または `Space` | 不正解として確定して次の問題へ |
| 「自己採点」モードでまだ答えを表示していない | `Enter` | 「答えを表示」を実行 |
| 答えを表示した後（自動判定なし） | `Enter` または `F` | 正解として確定して次の問題へ |
| 同上 | `J` または `Space` | 不正解として確定して次の問題へ |
