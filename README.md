...
- 手動追加セクション（JSON直接貼り付け／`.json` ファイルのドラッグ&ドロップまたは選択読み込み）はデフォルト非表示の開閉式（アコーディオン）に対応。必要な時にタップして展開可能
- シャッフル出題および回答モード切り替えのトグルボタンを大型化＆ON/OFFで明確に色付け（視認性と操作性を大幅向上）
...
2. 出題設定の選択
   - 大型トグルボタンで「シャッフル ON/OFF」や「回答モード（自己採点 / 入力採点）」を設定
3. 次のいずれかの方法で問題を読み込む
   - 「クイズセットを選ぶ」一覧から、カテゴリの下に並んだクイズセットを選ぶ（KVに登録されている場合に表示。クリックすると名前入力なしですぐ開始／前回の続きがあれば自動で再開）
   - 「＋ 自分でクイズを追加する」アコーディオンを開き、`.json` ファイルを選択／ドロップ、またはテキストエリアにJSONを直接貼り付けて「クイズ開始」を押す
